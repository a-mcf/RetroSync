...
2. **Path safety.** Member paths in `sync_members` (and picker/discovery browse paths) are user-supplied and joined against a node save-root via `safepath.Resolve`. Check for traversal (`..`), absolute-path escape, and symlink escape before any read/write/list. Reads off a local Syncthing share (and writes via a future SFTP adapter) both need this.
3. **Atomic writes & crash safety.** Per the state machine: copy to `<dest>.retrosync-tmp` then atomic rename; update `manifest` only *after* the rename. **Capture-before-overwrite (slice 20):** before any overwrite, the about-to-be-destroyed bytes are snapshotted to the server-side `save_versions` store (this replaced the old device-side `<path>.retrosync-conflict-<ts>` sibling backups) — verify a capture failure ABORTS the write so nothing recoverable is destroyed without a snapshot. Verify these hold wherever files are written.
4. **DB invariants.** `UNIQUE(sync_members.node_id, path)` ("one save file lives in exactly one sync" — the per-person isolation guarantee) must be enforced in the schema, not just in app code. The `save_versions` retention/restore must bind by `(sync_id, seq)` (no cross-sync restore). FKs and PKs match `docs/data-model.md`. Migrations are forward-only and idempotent enough to re-run in CI.
